...
import com.fuck.manspace.common.MyApplication;
...
/**
 * 使用方式:
 * final DiskCacheManager manager = new DiskCacheManager(context, SpaceConfigs.LOGIN_USER);
 *保存: manager.put(SpaceConfigs.LOGIN_USER, bean.getResult());
 *获取: LoginUser loginUser = manager.getSerializable(SpaceConfigs.LOGIN_USER);
 */
public class DiskCacheManager {
    ...
    public DiskCacheManager(Context context, String uniqueName) {
        try {
            ...
            mDiskLruCache = DiskLruCache.open(cacheFile, MyApplication.getAppVersionCode(), 1, SpaceConfigs.CACHE_MAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private File getCacheFile(Context context, String uniqueName) {
        ...
        if ((Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState()) || !Environment
                .isExternalStorageRemovable()) && context.getExternalCacheDir() != null) {
